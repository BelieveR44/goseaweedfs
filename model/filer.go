...
import (
	...
	"github.com/BelieveR44/goseaweedfs/libs"
)
...
// Upload a reader
func (f *Filer) Upload(reader io.Reader, fileName string, fileSize int64, newFilerPath string, collection string, ttl string) (result *FilerUploadResult, err error) {
	fp := NewFilePartFromReader(reader, fileName, fileSize)
	fp.Collection = collection
	fp.TTL = ttl
	...
	data, _, err := f.HTTPClient.Upload(f.URL+newFilerPath, fileName, fp.Reader, fp.IsGzipped, fp.MimeType)
	if err != nil {
		return
	}
